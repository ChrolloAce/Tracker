  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        // Portal URL is generated by an admin via the existing
        // /api/create-creator-share endpoint (admin-only) so signup itself
        // doesn't auto-issue a share token. The success screen tells the
        // creator their account exists; the admin shares the portal link.
      },
    });
  } catch (err: any) {
