...
const ApifyCostChart: React.FC<ApifyCostChartProps> = ({ dailyCosts }) => {
  if (dailyCosts.length === 0) {
    return (
      <div className="bg-surface-secondary border border-border-subtle rounded-xl p-6 text-center text-content-muted text-sm">
  ...
  return (
    <div className="bg-surface-secondary border border-border-subtle rounded-xl p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-4 text-[10px] text-content-muted">
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-content-secondary" /> Cost
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-content-muted" /> Runs
          </span>
        </div>
      </div>
      ...
      <div className="flex items-end gap-1 h-40">
        {dailyCosts.map((day) => {
          ...
          return (
            ...
            >
              {/* Tooltip */}
              <div className="absolute bottom-full mb-2 hidden group-hover:block z-10 bg-surface-tertiary border border-border rounded-lg p-2 text-[10px] text-content whitespace-nowrap shadow-xl">
                <div className="font-medium">{day.date}</div>
                <div className="text-content-secondary">Cost: ${day.totalUsd.toFixed(4)}</div>
                <div className="text-content-secondary">Runs: {day.runCount}</div>
              ...
              <div className="w-full flex gap-px items-end h-32">
                <div
                  className="flex-1 bg-content-secondary rounded-t transition-all duration-300"
                  style={{ height: `${Math.max(costPct, 2)}%` }}
                />
                <div
                  className="flex-1 bg-content-muted rounded-t transition-all duration-300"
                  style={{ height: `${Math.max(runsPct, 2)}%` }}
                />
              </div>

              {/* Date label */}
              <div className="text-[8px] text-content-muted mt-1 rotate-0">{dayLabel}</div>
            </div>
          );
        })}
      </div>
      ...
      {/* Summary row */}
      <div className="flex justify-between mt-3 pt-3 border-t border-border-subtle text-[10px] text-content-muted">
        <span>
