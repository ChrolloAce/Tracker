...
export default function FlowLines() {
  ...
  useLayoutEffect(() => {
    ...
    const compute = () => {
      ...
      [leftPts, rightPts].forEach((pts) => {
        ...
        const L = (tmp as any).getTotalLength();
        [0.25, 0.5, 0.75].forEach((f) => {
          const p = (tmp as any).getPointAtLength(L * f);
          chips.push({ x: p.x, y: p.y });
        });
      });
  ...
  // SVG size matches container
  const [box, setBox] = useState({ w: 0, h: 0 });
  useEffect(() => {
    const root = rootRef.current!;
    const update = () => setBox({ w: root.clientWidth, h: root.scrollHeight });
    update();
  ...
  return (
    ...
    >
      ...
      {/* Base paths (wider, faint) */}
      <path d={leftD} fill="none" stroke="rgba(79,70,229,.20)" strokeWidth="6" strokeLinecap="round" />
      <path d={rightD} fill="none" stroke="rgba(79,70,229,.20)" strokeWidth="6" strokeLinecap="round" />
      
      {/* Gradient paths (thinner, glowing) */}
      ...
      {/* Merge spine */}
      <path d={spineD} fill="none" stroke="rgba(79,70,229,.20)" strokeWidth="6" strokeLinecap="round" />
      <path d={spineD} fill="none" stroke="url(#g)" strokeWidth="3.5" strokeLinecap="round" filter="url(#glow)" />

      {/* Creator chips riding the rails */}
      {chipPts.map((p, i) => (
        <g key={i} transform={`translate(${p.x - 10},${p.y - 10})`}>
          <circle cx="10" cy="10" r="12" fill="#fff" />
          <circle cx="10" cy="10" r="8" fill="#e0e7ff" />
          <circle cx="10" cy="10" r="12" fill="none" stroke="url(#g)" strokeWidth="2" opacity=".7" />
        </g>
      ))}
    </svg>
  );
