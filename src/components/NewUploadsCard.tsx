...
const NewUploadsCard: React.FC<NewUploadsCardProps> = ({ 
  submissions, 
  dateRangeStart, 
  dateRangeEnd,
  onVideoClick 
  ...
  // Filter videos uploaded within the date range
  const newUploads = useMemo(() => {
    ...
    let filtered = Array.from(uniqueVideos.values());
    ...
    if (dateRangeStart && dateRangeEnd) {
      ...
      filtered = filtered.filter(video => {
        // Use uploadDate (actual upload date) if available, otherwise fall back to dateSubmitted
        const uploadDate = video.uploadDate || video.dateSubmitted;
        const videoDate = uploadDate instanceof Date ? uploadDate : new Date(uploadDate);
        
        return isWithinInterval(videoDate, { start, end });
      });
    }
    ...
    return filtered
      ...
      .slice(0, topCount);
  }, [submissions, selectedMetric, topCount, dateRangeStart, dateRangeEnd]);

  const maxMetricValue = newUploads.length > 0 ? getMetricValue(newUploads[0], selectedMetric) : 1;
  ...
  return (
    <div className="bg-zinc-900 rounded-lg shadow-lg border border-white/10 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-semibold text-white">New Uploads</h2>
      ...
      {newUploads.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-white/40 text-sm">
            {dateRangeStart && dateRangeEnd 
              ? 'No videos uploaded in this date range'
              : 'No videos found'
            }
          </p>
        </div>
        ...
        >
          <div className="bg-gray-900 text-white px-3 py-2 rounded-lg shadow-xl text-xs whitespace-nowrap">
            ...
            <div className="text-gray-300">
              Uploaded {formatUploadDate(hoveredVideo.video)}
            </div>
          </div>
        </div>,
        document.body
